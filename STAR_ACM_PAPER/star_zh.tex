...
Experiments on five benchmarks show that STAR achieves consistently higher Recall than QGpT on most datasets, with improvements of up to 17\% on R@1, demonstrating the effectiveness of semantic clustering and weighted fusion for robust table representation.
\end{abstract}
...
為了緩解這一問題，近期的研究開始探索利用大型語言模型（LLMs）生成合成問題（synthetic questions）來豐富表格的語義表示。其中，QGpT\cite{liang-etal-2025-improving-table}是一個代表性方法，它透過從表格中抽取部分列（partial table）並生成相關問題，試圖在查詢與表格之間建立語義橋樑。儘管QGpT取得了一定的成功，但我們發現其存在兩個關鍵缺陷：
...
\textbf{（1）語義聚類和查詢生成（SCDG）：}我們採用header-aware K-means聚類方法對表格列進行分組，從每個聚類中選取中心點實例構建partial table，確保選中的列既具有代表性又保持語義多樣性。不同於QGpT直接對整個partial table生成問題，我們對每個聚類分別生成synthetic question，確保生成的問題能夠全面覆蓋表格的語義空間。

\textbf{（2）加權融合（WF）：}我們將table embedding和queries embedding分別編碼，並提出兩種加權融合策略：Fixed Weight Fusion（FWF）透過固定權重平衡table和queries的重要性；Dynamic Weight Fusion（DWF）根據語義相似度自適應調整權重，實現更細粒度的語義整合。
...
\subsection{表格檢索}
表格檢索旨在從大規模語料庫中檢索與使用者查詢最相關的表格。早期方法主要基於稀疏檢索技術，如BM25\cite{robertson2009probabilistic}，但這些方法難以捕捉深層的語義關聯。隨著深度學習的發展，密集檢索方法\cite{karpukhin2020dense,izacard2022unsupervised}開始被應用於表格檢索任務。這些方法通常將表格線性化為文字序列，然後使用預訓練語言模型進行編碼。

為了更好地處理表格的結構特性，一些研究提出了結構感知的表格編碼方法。TaBERT\cite{yin2020tabert}透過設計專門的注意力機制來建模表格的行列結構。然而，這些方法往往需要大規模的預訓練，計算成本較高。

\subsection{查詢生成與資料增強}
查詢生成（Query Generation）是一種有效的資料增強技術，已被廣泛應用於資訊檢索和問答系統中。GPL\cite{wang2022gpl}提出使用生成式模型為文件生成偽查詢（pseudo queries），從而在無標註資料的情況下訓練檢索模型。InPars\cite{bonifacio2022inpars}進一步探索了使用大型語言模型進行查詢生成的可行性。

在表格檢索領域，QGpT\cite{liang-etal-2025-improving-table}首次系統地應用了LLM進行合成問題生成。該方法透過為partial table生成相關問題，試圖彌合查詢與表格之間的語義鴻溝。然而，QGpT採用簡單的top-k採樣策略和拼接式融合方法，限制了其表達能力。

\subsection{聚類在資訊檢索中的應用}
聚類技術被廣泛應用於資訊檢索中以提升檢索效率和效果。一些研究\cite{liu2004cluster}使用聚類方法對文件集合進行組織，從而加速檢索過程。在表格領域，聚類可以用於識別表格中的語義模式和冗餘資訊。本文首次將聚類技術應用於表格實例選擇，以提升partial table的代表性和多樣性。
...
QGpT採用簡單的top-k策略選取表格的前k列作為partial table，並直接對其生成synthetic queries。這種方法存在兩個問題：(1) 前k列可能語義相似，造成資訊冗餘；(2) 生成的問題可能偏向某些特定的語義方面，無法充分覆蓋表格的語義空間。為了解決這些問題，我們提出SCDG模組，結合語義聚類和聚類引導的查詢生成。

\textbf{Header-aware K-means Clustering:}
我們首先使用預訓練的embedding模型（如BGE-M3）分別對表頭 $\mathcal{H}$ 和每個表格列 $\mathbf{r}_i$ 進行編碼，得到header embedding $\mathbf{e}_{\mathcal{H}}$ 和row embedding $\mathbf{e}_{\mathbf{r}_i}$：
\begin{align}
...
\end{equation}
其中 $\alpha$ 是控制header和row相對重要性的超參數。這種設計使得每個row的表示既包含其自身的語義資訊，也融合了表頭的結構資訊。
...
QGpT採用簡單的拼接策略，將partial table和synthetic queries拼接後送入編碼器。這種方法將不同資訊來源混合在一起，缺乏對各部分語義重要性的建模。我們提出加權融合策略，分別編碼table和queries，然後進行加權組合。
...
給予table固定權重 $w_t = \alpha$，剩餘權重分配給queries：$w_q = 1-\alpha$。這種方法簡單直觀，透過調整 $\alpha$ 可以控制table和queries在最終表示中的相對重要性。
...
\end{equation}
其中 $\beta$ 是控制query embedding權重範圍的超參數。當table和queries語義相似度高時，賦予queries更大的權重；反之則更依賴table本身的表示。
...
(1) \textbf{BGE-M3}\cite{xiao2023cpack}：先進的多語言密集檢索模型，直接對表格進行編碼；
(2) \textbf{QGpT}\cite{liang-etal-2025-improving-table}：使用LLM生成synthetic queries進行表格檢索的方法。

\textbf{實現細節：}我們遵循QGpT的實驗設置。使用BGE-M3作為embedding模型，使用Llama 3.1 8B-Instruct作為synthetic query generation的生成模型。聚類數量 $k$ 設置為10，因此為每個表格生成10個queries。在header-aware K-means clustering中，我們設置 $\alpha=0.2$；在Fixed Weight Fusion中，我們設置table權重為0.7；在Dynamic Weight Fusion中，我們設置 $\beta=0.3$。所有實驗使用Recall@1、Recall@5和Recall@10作為評估指標。

\subsection{主要實驗結果}

表\ref{tab:main_results}展示了STAR與QGpT在五個資料集上的比較結果。我們可以觀察到以下幾點：

\textbf{(1) STAR顯著優於QGpT：}在大多數資料集上，STAR的兩個變體都顯著優於QGpT baseline。特別是在Mimo (en)資料集上，STAR w/ DWF在R@1上達到58.89\%，相比QGpT的50.66\%提升了8.23個百分點。在E2E-WTQ資料集上，STAR w/ FWF在R@1上達到58.51\%，相比QGpT的41.49\%提升了17.02個百分點。這驗證了我們提出的語義聚類和加權融合策略的有效性。

\textbf{(2) FWF與DWF的比較：}兩種加權融合策略在不同資料集上表現相近。Dynamic Weight Fusion在大多數情況下略優於Fixed Weight Fusion，這表明根據語義相似度自適應調整權重確實有效。然而，Fixed Weight Fusion憑藉其簡單性和穩定性，在某些資料集上也取得了競爭力的結果。
...
(4) \textbf{QGpT}：原始QGpT方法作為baseline。
...
\textbf{(3) 整體提升：}完整的STAR模型相比QGpT baseline在平均R@1上提升了6.39個百分點（從45.47\%提升至51.86\%），證明了兩個核心模組的協同效果。
...
這些結果為未來研究learned weighting機制提供了方向，即透過端到端訓練學習權重分配策略。
...
未來的研究方向包括：(1) 探索端到端的learned weighting機制，透過訓練自動學習最優權重分配；(2) 研究如何動態確定最優的聚類數量 $k$；(3) 將STAR擴展到多模態表格檢索任務；(4) 探索STAR與表格問答等下游任務的結合。
